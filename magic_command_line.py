...
from run_magic import run_magic_from_file
...
def parse_args():
    ...
    kernel_group.add_argument('-k', '--knn', type=int, default=10, dest='knn',
    ...
    magic_group.add_argument('--t-magic', type=str, default='auto',
                             help='Level of diffusion for MAGIC',
                             metavar='T')
    genes = magic_group.add_mutually_exclusive_group()
    genes.add_argument('--pca-only', action='store_true', default=False,
                       help='Return PCA on the smoothed matrix')
    genes.add_argument('--all-genes', action='store_true', default=False,
                       help='Return the entire smoothed matrix')
    genes.add_argument('--gene-list', type=str, nargs='+', default=None,
                       help='List of genes to return from MAGIC, '
                       'either as integer indices or column names.',
                       metavar='GENE', dest='genes')
    magic_group.add_argument('--output', type=str, default='magic.csv',
                             help='Output CSV file to save smoothed '
                             'data matrix', metavar='FILE')
    ...
    if args.t_magic != 'auto':
        try:
            ...
            parser.error(
                "argument --t-magic: invalid int value: '{}'".format(args.t_magic))
    # fix debug argument
    if args.debug:
        args.verbose = 2
    del args.debug
...
if __name__ == "__main__":
    ...
    tasklogger.log_debug(
        "Running MAGIC with arguments {}".format(args.__dict__))
